...
import { validateForecasterDesc } from '../validation';
...
describe('validations', () => {
  describe('validateDetectorDesc', () => {
    ...
    test('should throw size limit if exceed  400', () => {
      expect(
        validateForecasterDesc(descriptionGenerator.paragraph({ length: 500 }))
      ).toEqual('Description Should not exceed 400 characters');
    });
    test('should return undefined if not empty', () => {
      expect(validateForecasterDesc('This is description')).toBeUndefined();
    });
  });
});
