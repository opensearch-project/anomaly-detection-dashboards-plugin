...
describe('GenerateAnomalyDetector spec', () => {
            ...
            (getAssistantClient().executeAgentByConfigName as jest.Mock).mockResolvedValueOnce({
                body: {
                    inference_results: [
                        {
                            output: [
                                { result: "{\"index\":\"test-pattern\",\"categoryField\":\"\",\"aggregationField\":\"responseLatency\",\"aggregationMethod\":\"avg\",\"dateFields\":\"timestamp,nested.utc_time\"}" }
                            ]
                        }
                    ]
                }
            ...
            (getAssistantClient().executeAgentByConfigName as jest.Mock).mockResolvedValueOnce({
                body: {
                    inference_results: [
                        {
                            output: [
                                { result: "{\"index\":\"test-pattern\",\"categoryField\":\"\",\"aggregationField\":\"responseLatency\",\"aggregationMethod\":\"avg\",\"dateFields\":\"nested.timestamp,nested.utc_time\"}" }
                            ]
                        }
                    ]
                }
    ...
    describe('Test agent not found', () => {
        ...
        it('shows danger toast when agent does not exist', async () => {
            (getAssistantClient().agentConfigExists as jest.Mock).mockResolvedValueOnce({ exists: false });
        ...
        it('shows danger toast when generated feature list is empty', async () => {
            (getAssistantClient().agentConfigExists as jest.Mock).mockResolvedValueOnce({ exists: true });
            (getAssistantClient().executeAgentByConfigName as jest.Mock).mockResolvedValueOnce({
                body: {
                    inference_results: [
                        {
                            output: [
                                { result: "{\"index\":\"test-pattern\",\"categoryField\":\"\",\"aggregationField\":\"\",\"aggregationMethod\":\"\",\"dateFields\":\"timestamp\"}" }
                            ]
                        }
                    ]
                }
    ...
    describe('Test UI interactions on rendered form', () => {
            ...
            (getAssistantClient().executeAgentByConfigName as jest.Mock).mockResolvedValueOnce({
                body: {
                    inference_results: [
                        {
                            output: [
                                { result: "{\"index\":\"test-pattern\",\"categoryField\":\"ip\",\"aggregationField\":\"responseLatency,response\",\"aggregationMethod\":\"avg,sum\",\"dateFields\":\"@timestamp\"}" }
                            ]
                        }
                    ]
                }
        ...
        it('shows danger toast when Create button is clicked while featureList is empty', async () => {
            ...
            (getAssistantClient().executeAgentByConfigName as jest.Mock).mockResolvedValueOnce({
                body: {
                    inference_results: [
                        {
                            output: [
                                { result: "{\"index\":\"test-pattern\",\"categoryField\":\"\",\"aggregationField\":\"responseLatency\",\"aggregationMethod\":\"avg\",\"dateFields\":\"@timestamp\"}" }
                            ]
                        }
                    ]
                }
            ...
            httpClientMock.get = jest.fn().mockResolvedValue({
                ...
                response: {
                    mappings: {
                        test: { mappings: { properties: { '@timestamp': { type: 'date' } } } }
                    }
                },
            });
    ...
    describe('Test create detector — max detectors reached', () => {
            ...
            (getAssistantClient().executeAgentByConfigName as jest.Mock).mockResolvedValueOnce({
                body: {
                    inference_results: [
                        {
                            output: [
                                { result: "{\"index\":\"test-pattern\",\"categoryField\":\"ip\",\"aggregationField\":\"responseLatency\",\"aggregationMethod\":\"avg\",\"dateFields\":\"@timestamp\"}" }
                            ]
                        }
                    ]
                }
        ...
        it('shows max-detectors danger toast when detector limit is reached', async () => {
            ...
            await userEvent.setup().click(getByTestId('SuggestAnomalyDetectorCreateButton'));
    ...
    describe('Test custom result index and category field toggles', () => {
        ...
        it('opens Advanced configuration accordion and toggles custom result index checkbox', async () => {
            ...
            await user.click(getByText('Advanced configuration'));

            // Toggle custom result index checkbox on
            const resultIndexCheckbox = getByLabelText('Enable custom result index');
            await user.click(resultIndexCheckbox);
            expect(queryByText("You can't change the custom result index after you create the detector. You can manage the result index with the Index Management plugin.")).not.toBeNull();
        ...
        it('opens Advanced configuration accordion and toggles categorical field checkbox', async () => {
            ...
            await user.click(categoryFieldCheckbox);
            // Toggle it back on
            await user.click(categoryFieldCheckbox);
        });
    });
